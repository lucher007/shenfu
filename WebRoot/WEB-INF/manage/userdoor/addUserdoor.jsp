...
<body>
		<div class="form-box">
			<form method="post"  enctype="multipart/form-data" id="addForm" name="addForm" >
				<input id="ordercode" name="ordercode" type="hidden" value="${userdoor.ordercode}"/> 
				<div class="easyui-panel" title="门锁信息" style="width:100%;" data-options="footer:'#ft'">
	...
	function checkVal() {
		if ($("#ordercode") != undefined && ($("#ordercode").val() == "" || $("#ordercode").val() == null )) {
			$.dialog.tips("请选择订单编号", 1, "alert.gif", function() {
				$("#ordercode").focus();
			});
			return false;
		}
	...
	function saveUser() {
		...
		$("#addForm").attr("action", "userdoor/save");
	    $("#addForm").submit();
	}
    ...
    function plupload(){
		...
		$("#uploader").plupload({
			...
			url : '<%=basePath%>userdoor/saveUserdoorInfo',
			 // 参数  
            multipart_params: {	
            					'ordercode': ordercode, 
                              },
