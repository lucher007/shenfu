...
<body>
		    	...
		    	<tr>
		    		...
		    		<td>
		    			<select id="status" name="status" class="select" onchange="queryUserorder();">
		    			    <option value="6" <c:if test="${userorder.status == '6' }">selected</c:if>>安装完成</option>
			             </select>
		    		</td>
    ...
    <div id = "userorderDG" style="width:auto; height: 400px"></div>
    <div id="tools">
		<div style="margin-bottom:5px">
			<a href="javascript:confirmFirstArrival();" class="easyui-linkbutton" iconCls="icon-ok"  plain="true">确认尾款到账</a>
  ...
  //合同查看
	...
	*/
	function confirmFirstArrival(){
		...
		$.dialog.confirm('确认尾款到账？', function(){ 
			var data = {
					     id: id
					   };
			var url="userorder/updateFinalArrival?rid="+Math.random();
			$.getJSON(url,data,function(jsonMsg){
