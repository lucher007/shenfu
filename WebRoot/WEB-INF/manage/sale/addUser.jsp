...
<body>
	<div id="body">
		<form method="post" id="addForm" name="addForm">
			...
			<div class="easyui-panel" style="width:100%;" data-options="footer:'#ft'">
				<div class="easyui-panel" title="订户信息" style="width:1040px;">	
					<table width="100%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							<td height="30" width="15%" align="right">客户姓名：</td>
							<td width="25%"><input type="text" class="inp" name="username" id="username" value="${user.username }"> <span class="red">*</span></td>
						</tr>
						<tr>
							<td height="30" width="15%" align="right">联系电话：</td>
							<td width="25%"><input type="text" class="inp" name="phone" id="phone" value="${user.phone }" maxlength="12" onkeyup="checkNum(this)" onkeypress="checkNum(this)" onblur="checkNum(this)" onafterpaste="this.value=this.value.replace(/\D/g,'')">
						...
						<tr>
							...
							<td>
								<select id="source" name="source" class="select">
									...
									<!--
									<option value="1" <c:if test="${user.source == '1'}">selected</c:if>>400电话</option>
									  -->
								</select>
							</td>
						</tr>
						<tr>
							...
							<td width="25%"><input type="text" class="inp" style="width: 350px" name="address" id="address" value="${user.address }"></td>
						</tr>
						<tr>
						...
						<tr style="height: 10px">
							...
							<td width="85%">
								 <textarea id="dealresult" name="dealresult" style="width:450px; height:60px;"
		                       			onKeyDown="checkLength('dealresult',300)" onKeyUp="checkLength('dealresult',300)">${user.dealresult}</textarea>
				...
				<!--不需要体验需要显示的内容 -->
				<div id="modelDiv" style="width:1040px;">
				...
				<div id="otherproduct" style="width:1040px;">
							...
							<tr>
								 ...
								 <td >
									<select id="lockcoreflag" name="lockcoreflag" class="select" style="width: 200px;">
										<option value="0" <c:if test="${userorder.lockcoreflag == '0'}">selected</c:if>>不需要</option>
										<option value="1" <c:if test="${userorder.lockcoreflag == '1'}">selected</c:if>>需要</option>
									</select>
			   ...
			   <div id="visitorDiv" style="width:1040px;">
	...
	function checkVal() {
		...
		if ($("#salercode") != undefined && ($("#salercode").val() == "" || $("#salercode").val() == null )) {
			$("#salercode").focus();
			$.dialog.tips("请选择销售员", 1, "alert.gif");
			return false;
		}
	...
	function saveUser() {
		...
		$.dialog({
		    ...
		    icon: 'alert.gif',
		    ok: function () {
		    	$("#addForm").attr("action", "user/save?rid="+Math.random());
				$("#addForm").submit();
	...
	function chooseEmployee() {
		...
		var url = "";
		if(visittype == '0'){
			url = 'url:employee/findSalerListDialog?rid='+Math.random()
		}else if(visittype == '2'){
			url = 'url:employee/findInstallerListDialog?rid='+Math.random()
		}
	...
	function closeEmployeeDialog(jsonStr) {
		...
		if(saleremployeeDialog != undefined && saleremployeeDialog != null){
			 $("#salercode").val(jsonObject.employeecode);
			 $("#salername").val(jsonObject.employeename);
			 saleremployeeDialog.close();
			 saleremployeeDialog = null;
		}
		
		if(employeeDialog != undefined && employeeDialog != null){
	...
	$('#modelcode').combogrid({
        panelWidth: 850,
			idField: 'modelcode',
			textField: 'modelname',
			url: 'productmodel/getProductmodelCombogridJson?rid='+Math.random(),
			method: 'get',
			fitColumns: true,
			columns: [[
