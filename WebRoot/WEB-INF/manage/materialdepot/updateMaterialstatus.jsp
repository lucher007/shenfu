...
<body>
	<div id="body">
		<form method="post" id="addForm" name="addForm">
			...
			<div class="easyui-panel" title="材料库存信息" style="width:100%;" data-options="footer:'#ft'">
				<table style="width:100%">
					...
					<tr>
						...
						<td>
							<input type="text" class="inp w200" name="materialidentfy" id="materialidentfy" readonly="readonly" style="background:#eeeeee;" value="${materialdepot.materialdepot.materialidentfy }">
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							<select id="materialstatus" name="materialstatus" class="select">
								<option value="1" <c:if test="${materialdepot.materialdepot.materialstatus == '1'}">selected</c:if>>正常使用</option>
								<option value="2" <c:if test="${materialdepot.materialdepot.materialstatus == '2'}">selected</c:if>>已损坏</option>
								<option value="3" <c:if test="${materialdepot.materialdepot.materialstatus == '3'}">selected</c:if>>维修中</option>
							</select>
						</td>
					</tr>
					<tr>
						<td align="right">材料软件版本：</td>
						<td>
							<input class="inp" id="materialversion" name="materialversion" style="width: 400px;" value="${materialdepot.materialdepot.materialversion }">
						</td>
					</tr>
				</table>
			</div>
...
<script type="text/javascript">
  ...
  function saveOutdepot() {
		...
		$.dialog({
		    ...
		    icon: 'alert.gif',
		    ok: function () {
		    	$("#addForm").attr("action", "materialdepot/updateMaterialstatus?rid="+Math.random());
				$("#addForm").submit();
