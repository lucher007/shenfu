...
<body>
		...
		<form method="post" id="addForm" name="addForm">
			<div class="form-box">
				...
				<div class="fb-bom">
				    ...
				    <span class="red">${componentdepot.returninfo}</span>
				</div>
			</div>
		</form>
	</div>
	...
	function checkVal() {
		...
		if ($('#componentid').combobox('getValue') == "") {
			$.dialog.tips("请选择出库元器件", 1, "alert.gif", function() {
				$("#componentid").focus();
			});
			return false;
		}
		...
		if ($("#depotamount") != undefined && ($("#depotamount").val() == "" || $("#depotamount").val() == null )) {
			$.dialog.tips("请输入元器件出库量", 1, "alert.gif", function() {
				$("#depotamount").focus();
			});
			return false;
		}
	...
	$('#componentcode').combobox({    
	    url:'component/getMaterialComboBoxJson',    
	    valueField:'id',    
	    textField:'text',
	    //数据执行之后才加载
