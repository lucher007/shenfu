...
<body>
		...
		<form method="post" id="addForm" name="addForm">
			<div class="easyui-panel" title="生产组装信息" style="width:100%;">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						...
						<td width="20%">
							<input id="productcode" name="productcode" class="easyui-combogrid"> <span class="red">*</span>
						</td> 
						<td align="right" width="10%">产品名称：</td>
						<td width="15%">
							<input type="text" class="inp" name="productname" id="productname" readonly="readonly" style="background:#eeeeee;"> <span class="red">*</span>
						</td>
					...
					<tr>
						...
						<td width="15%">
							<input type="text" class="inp w200" name="producercode" id="producercode" readonly="readonly" style="background:#eeeeee;" value="${produceinfo.producercode }">
						...
						<td width="15%" colspan="3">
							<input type="text" class="inp w200" name="producername" id="producername" readonly="readonly" style="background:#eeeeee;" value="${produceinfo.producername }"><span class="red">*</span>
						</td>
					</tr>
					<tr>
						<td align="right" width="10%">产品版本：</td>
						<td width="15%">
							<input type="text" class="inp" style="width: 200px" name="productversion" id="productversion" value="${produceinfo.productversion}">
						</td>
					</tr>
					<tr>
						<td align="right">组装信息：</td>
			            <td colspan="5"> 
							<textarea id="installinfo" name="installinfo" style="width:550px; height:50px;"
		                       onKeyDown="checkLength('installinfo',100)" onKeyUp="checkLength('installinfo',100)">${produceinfo.installinfo}</textarea>
		            		<span class="red">还可以输入<span id="validNuminstallinfo">100</span>字</span>
		               </td>
					</tr>
				</table>
			</div>
			...
			<div id="ft" style="padding:5px;">
					<cite> 
						...
						<a href="javascript:saveProduceinfo();" class="easyui-linkbutton" iconCls="icon-save" style="width:80px">确定生产</a>
  ...
  function checkVal() {
		...
		if ($('#productcode').combogrid('getValue')=='') {
			$('#productcode').combogrid('textbox').focus();  
			$.dialog.tips("请选择产品编号", 2, 'alert.gif');
			return false;
	    }
  ...
  function saveProduceinfo() {
		...
		$.dialog({
		    ...
		    icon: 'alert.gif',
		    ok: function () {
		    	$("#addForm").attr("action", "produceinfo/save?rid="+Math.random());
				$("#addForm").submit();
    ...
    function initBomDatagrid(){
        $('#bomDG').datagrid({
            ...
            url:'produceinfo/findBomListJson',
            pagination: false,
            singleSelect: true,
            scrollbar:true,
            rownumbers:true,
            ...
            columns: [[
                       ...
                       { field: 'batchno', title: '材料批次号',width:100,align:"center",
                       	  editor:{
									...
									options:{
										...
										textField:'text',
										method:'get',
										url:'produceinfo/getMaterialdepotComboBoxJsonGroupByMaterialcode',
									},
    		                  },
                       },
                   ...
                   onBeginEdit :function(index,row){
                	   ...
                	   //重新加载下拉框的数据
                	   $(ed.target).combobox('reload', {materialcode:row.materialcode,depotstatus:'1',materialstatus:'1'});//如果datagrid数据源列名称不是material_id记得修改这里
                	   $(ed.target).combobox({
      ...
      //将表单数据转为json
  	function paramsJson(){
  	 	var productcode = $('#productcode').combogrid('getValue');
  	 	var json = '';
 		   ...
 		   //绑定回车事件
      	    ...
      	    $('.datagrid-editable .textbox,.datagrid-editable .datagrid-editable-input,.datagrid-editable .textbox-text').bind('keydown', function(e){  
                    ...
                    if(code == 13){
				        ...
				        var data = {          
			        			          materialcode:rows[index].materialcode,
			        			          materialname:rows[index].materialname,
			        			           inoutamount:rows[index].totalamount,
			        			          batchno:batchno,
								   };
						var url="produceinfo/checkMaterialdepotamount?rid="+Math.random();
	...
	var wireRod;  
    $.ajax({
        url: 'product/getProductCombogridJson?rid='+Math.random()+'&productsource=0',
        type: "post",  
        dataType: "json",  
