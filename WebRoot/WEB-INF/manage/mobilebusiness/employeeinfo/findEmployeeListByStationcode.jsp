...
<body>
	...
	<input type="hidden" name="extendcode" id="extendcode" value="${cellstationemployee.extendcode}"/>
	<header data-am-widget="header" class="am-header am-header-default am-header-fixed">
	       <div class="am-header-left am-header-nav">
	          <a href="mobilebusiness/lookCellstation?extendcode=${cellstationemployee.extendcode}" class="">
	              <img class="am-header-icon-custom" src="data:image/svg+xml;charset&#x3D;utf-8,&lt;svg xmlns&#x3D;&quot;http://www.w3.org/2000/svg&quot; viewBox&#x3D;&quot;0 0 12 20&quot;&gt;&lt;path d&#x3D;&quot;M10,0l2,2l-8,8l8,8l-2,2L0,10L10,0z&quot; fill&#x3D;&quot;%23fff&quot;/&gt;&lt;/svg&gt;" alt=""/>
	              <span class="am-header-nav-title">返回 </span>
	          </a>
	       </div>
	       <h1 class="am-header-title">驻点人员信息</h1>
	</header>
	<div class="content">
	    ...
	    var dropload = $('#content').dropload({
	        ...
	        loadDownFn : function(me){
            	...
            	var data = {
            					page:page_0,  //页码
            					rows:rows,    //每页显示行数
            					stationcode:$('#stationcode').val(),     //提取编号
						   };
                $.ajax({
                    type: 'GET',
                    url: 'mobilebusiness/findEmployeeListJSONByStationcode',
                    data: data, //可选参数
                    dataType: 'json',
                    success: function(data){
                        var result = '';
                        page_0++;
                        var listdata = data.cellstationemployeelist;
                        if(listdata.length > 0){
                        	for(var i = 0; i < listdata.length; i++){
                            	 result +=   '<tr class="lb-list">'
                            	         +     '<td>'+ listdata[i].employeecode + '<input type="hidden" value="'+ listdata[i].employeecode+'">' + '</td>'
                            	         +     '<td>'+ listdata[i].employeename +'</td>'
                            	         +     '<td>'+ listdata[i].phone +'</td>'
                            	         +     '<td>'+ listdata[i].employeetypename +'</td>'
	                                     +   '</tr>';
                            }
                            // 为了测试，延迟1秒加载
                            setTimeout(function(){
