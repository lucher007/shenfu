...
<body>
	...
	<header data-am-widget="header" class="am-header am-header-default am-header-fixed">
	       ...
	       <h1 class="am-header-title">抢购的楼盘信息</h1>
	</header>
	<div class="content">
	...
	$(function(){
	    ...
	    var dropload = $('#content').dropload({
	        ...
	        loadDownFn : function(me){
                ...
                $.ajax({
                    type: 'GET',
                    url: 'mobilebusiness/findMyCellextendListJSON',
                    data: data, //可选参数
                    dataType: 'json',
                    success: function(data){
                        var result = '';
                        page_0++;
                        var listdata = data.cellextendlist;
                        if(listdata.length > 0){
                        	for(var i = 0; i < listdata.length; i++){
                        		 result +=   '<li>'
                                     +'<a href="mobilebusiness/lookCellextend?extendcode='+ listdata[i].extendcode + '" class="am-list-item-hd" style="font-size:15px;">'+ listdata[i].cellname +'</a>'
                                     +'<div class="am-text-truncate">'+listdata[i].address+'</div>'
                                 	+'</li>';
                            }
                            // 为了测试，延迟1秒加载
                            setTimeout(function(){
