...
<body>
	...
	<input type="hidden" name="gainlogcode" id="gainlogcode" value="${salegaininfo.gainlogcode}"/>
	<header data-am-widget="header" class="am-header am-header-default am-header-fixed">
	       <div class="am-header-left am-header-nav">
	          <a href="mobilebusiness/findSalegainlogList" class="">
	              <img class="am-header-icon-custom" src="data:image/svg+xml;charset&#x3D;utf-8,&lt;svg xmlns&#x3D;&quot;http://www.w3.org/2000/svg&quot; viewBox&#x3D;&quot;0 0 12 20&quot;&gt;&lt;path d&#x3D;&quot;M10,0l2,2l-8,8l8,8l-2,2L0,10L10,0z&quot; fill&#x3D;&quot;%23fff&quot;/&gt;&lt;/svg&gt;" alt=""/>
	              <span class="am-header-nav-title">返回 </span>
	          </a>
	       </div>
	       <h1 class="am-header-title">未领提成记录</h1>
	</header>
	<div class="content">
	    ...
	    var dropload = $('#content').dropload({
	        ...
	        loadDownFn : function(me){
            	...
            	var data = {
            					...
            					rows:rows,    //每页显示行数
            					status:1,     //未领取的提成
            					gainlogcode:$('#gainlogcode').val(),     //提取编号
						   };
                $.ajax({
                    type: 'GET',
                    url: 'mobilebusiness/findSalegaininfoListJSONByGainlogcode',
                    data: data, //可选参数
                    dataType: 'json',
                    success: function(data){
                        var result = '';
                        page_0++;
                        var listdata = data.salegaininfolist;
                        if(listdata.length > 0){
                        	for(var i = 0; i < listdata.length; i++){
                            	 result +=   '<tr class="lb-list">'
                            	         +     '<td>'+ listdata[i].username + '<input type="hidden" value="'+ listdata[i].usercode+'">' + '</td>'
                            	         +     '<td>'+ listdata[i].gainname +'</td>'
                            	         +     '<td>'+ listdata[i].addtime +'</td>'
                            	         +     '<td>'+ listdata[i].gainmoney +'</td>'
	                                     +   '</tr>';
                            }
                            // 为了测试，延迟1秒加载
                            setTimeout(function(){
