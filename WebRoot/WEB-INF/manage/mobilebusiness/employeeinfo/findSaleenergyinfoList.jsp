...
<body>
	...
	<input type="hidden" name="status" id="status" value="${saleenergyinfo.status}"/>
	<header data-am-widget="header" class="am-header am-header-default am-header-fixed">
	       ...
	       <h1 class="am-header-title">
	       		<c:choose>  
				    <c:when test="${saleenergyinfo.status == '0'}">未兑换行动力</c:when> 
				    <c:when test="${saleenergyinfo.status == '1'}">已兑换行动力</c:when> 
				    <c:otherwise>
				    	行动力
				    </c:otherwise>  
				 </c:choose>
	    ...
	    var dropload = $('#content').dropload({
	        ...
	        loadDownFn : function(me){
            	...
            	var data = {
            					page:page_0,  //页码
            					rows:rows,    //每页显示行数
            					status:$('#status').val(),     //提取编号
						   };
                $.ajax({
                    type: 'GET',
                    url: 'mobilebusiness/findSaleenergyinfoListJSON',
                    data: data, //可选参数
                    dataType: 'json',
                    success: function(data){
                        var result = '';
                        page_0++;
                        var listdata = data.saleenergyinfolist;
