...
<body>
	<form class="am-form am-form-horizontal" id="addForm" method="post" >
	    <input type="hidden" name="id" id="id" value="${userorder.usertask.id}"/>
		<input type="hidden" name="taskcode" id="taskcode" value="${userorder.usertask.taskcode}"/>
		<header data-am-widget="header" class="am-header am-header-default am-header-fixed">
	       <div class="am-header-left am-header-nav">
	          <a href="mobilebusiness/manageUsertask" class="">
	              <img class="am-header-icon-custom" src="data:image/svg+xml;charset&#x3D;utf-8,&lt;svg xmlns&#x3D;&quot;http://www.w3.org/2000/svg&quot; viewBox&#x3D;&quot;0 0 12 20&quot;&gt;&lt;path d&#x3D;&quot;M10,0l2,2l-8,8l8,8l-2,2L0,10L10,0z&quot; fill&#x3D;&quot;%23fff&quot;/&gt;&lt;/svg&gt;" alt=""/>
	              <span class="am-header-nav-title">返回 </span>
	          </a>
	       </div>
	       <h1 class="am-header-title">任务单详情</h1>
	  	</header>
	  	<div class="am-form-group">
			<label for="taskcode" class="am-form-label am-u-sm-3" style="text-align: right;">任务单编号:</label>
			<div class=" am-u-sm-9" align="right">
				${userorder.usertask.taskcode}
			</div>
		</div>
	  	<div class="am-form-group">
	      	 ...
	      	 <div class="am-u-sm-9">
	      	 	 <select  id="modelcode" name="modelcode">
			         ...
			         <c:forEach items="${productmodelList}" var="productmodel">  
		               <option value="${productmodel.modelcode}" <c:if test="${userorder.modelcode eq productmodel.modelcode}">selected</c:if>>${productmodel.modelname}</option>
			        </c:forEach> 
			     </select>
		...
		<div class="am-form-group">
			...
			<div class="am-form-content am-u-sm-9">
				<select id="productcolor" name="productcolor"">
				  <option value="">请选择</option>
				  <option value="0101" <c:if test="${userorder.productcolor eq '0101'}">selected</c:if>>摩卡棕</option>
				  <option value="0102" <c:if test="${userorder.productcolor eq '0102'}">selected</c:if>>石英灰</option>
				  <option value="0103" <c:if test="${userorder.productcolor eq '0103'}">selected</c:if>>香槟金</option>
				</select>
			</div>
		</div>
		<div class="am-form-group">
			...
			<div class="am-form-content am-u-sm-9">
					<select id="lockcoreflag" name="lockcoreflag">
					  <option value="0" <c:if test="${userorder.lockcoreflag eq '0'}">selected</c:if>>不需要</option>
					  <option value="1" <c:if test="${userorder.lockcoreflag eq '1'}">selected</c:if>>需要</option>
					</select>
		...
		<div class="info-btn">
			<button type="button" class="am-btn am-alert am-btn-block" style="height: 50px;" onclick="javascript:saveUserorder_forUsertask();">提交</button>
		</div>
		<div style="height: 15px;"></div>
	...
	var ele=[];
	function saveUserorder_forUsertask() {
		if (!checkVal()) {
			return;
		}
        ...
        var onConfirm = function() {
        	//如果正在提交则直接返回，停止执行
	          ...
	          $.ajax({
	  	          url: "mobilebusiness/saveUserorder_forUsertask?rid="+Math.random(),  
	  	          type: 'POST',  
	  	          data: formData,  
	  	          async: true,  
	  	          cache: false,  
