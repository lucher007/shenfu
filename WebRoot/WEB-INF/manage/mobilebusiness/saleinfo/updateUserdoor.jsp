...
<body>
	<form method="post" id="addForm" name="addForm" class="am-form am-form-horizontal">
		<input type="hidden" name="ordercode" id="ordercode" value="${userdoor.userorder.ordercode}"/>
		<input type="hidden" name="jumpurl" id="jumpurl" value="${userdoor.jumpurl}"/>
		<header data-am-widget="header" class="am-header am-header-default am-header-fixed">
		       <div class="am-header-left am-header-nav">
		          <a href="${userdoor.jumpurl}?ordercode=${userdoor.userorder.ordercode}" class="">
		...
		<div class="am-form-group" >
			<label for="username" class="am-form-label am-u-sm-10">已有门锁图片信息：</label>
		</div>
		
		<!-- 已有图片信息 -->
		<ul data-am-widget="gallery" class="am-gallery am-avg-sm-2 am-gallery-imgbordered" data-am-gallery="{pureview: 1}" id="userdoor"></ul>
		
		<hr data-am-widget="divider" style="" class="am-divider am-divider-default" />
		<div class="am-form-group">
			<label for="user-name2" class="am-form-label am-u-sm-3" style="text-align: right;">新增加门锁图片</label>
		...
		<div class="info-btn">
			<button type="button" class="am-btn am-alert am-btn-block" style="height: 50px;" onclick="javascript:saveUserdoorInfo();">上传</button>
		</div>
		<div style="height: 15px;"></div>
	...
	<script type="text/javascript">
		...
		function initUserdoorList() {
        	...
        	var data = {
			 ...
			 $.ajax({
				  type: 'GET',
                  url: 'mobilebusiness/findUserdoorListJSON',
                  data: data, //可选参数
                  dataType: 'json',  
	  	          success: function (data) {
	  	        	  var result = '';
	  	        	  var dataList = data.userdoorList;
	  	        	  if(dataList.length > 0){
                    	  for(var i = 0; i < dataList.length; i++){
                        	   result +=   ' <li>'
                        		       +        '<div class="am-gallery-item">' 
                        	           +            '<img src="mobilebusiness/findUserdoor?id='+ dataList[i].id + '"/>' 
                        	           +            '<h3 class="am-gallery-title" onclick="javascript:deleteUserdoor(' + dataList[i].id + ');">点击删除</h3>'
                        	           +        '</div>'
                                       +   '</li>';
                          }
                      }
	  	        	  //给图片DIV赋值
	  	        	  $('#userdoor').html(result);
	  	          },  
	  	     });
		}
		...
		function deleteUserdoor(id) {
	        ...
	        var onConfirm = function() {
	        	...
	        	var data = {
				 ...
				 $.ajax({
					  type: 'GET',
	                  url: 'mobilebusiness/deleteUserdoor',
	                  data: data, //可选参数
	                  dataType: 'json',  
		  	          success: function (data) {
		...
		function saveUserdoorInfo() {
	        ...
	        var onConfirm = function() {
	        	//如果正在提交则直接返回，停止执行
		          ...
		          //客户姓名
		          formData.append("ordercode",$("#ordercode").val());
		          
		          $("<div class='loadingWrap'></div>").appendTo("body");  
		          $.ajax({
		  	          url: "mobilebusiness/saveUserdoorInfo?rid="+Math.random(),  
		  	          type: 'POST',  
		  	          data: formData,  
		  	          async: true,  
		  	          cache: false,  
