...
<body>
	...
	$(function(){
	    ...
	    var dropload = $('#content').dropload({
	        ...
	        loadDownFn : function(me){
                ...
                $.ajax({
                    type: 'GET',
                    url: 'mobilebusiness/findUserdispatchListJSON',
                    data: data, //可选参数
                    dataType: 'json',
                    success: function(data){
                        var result = '';
                        page_0++;
                        var listdata = data.userdispatchlist;
                        if(listdata.length > 0){
                        	for(var i = 0; i < listdata.length; i++){
                            	 result +=   '<tr class="lb-list">'
                            	         +     '<td>'+ listdata[i].dispatchcode + '<input type="hidden" value="'+ listdata[i].dispatchcode+'">' + '</td>'
                            	         +     '<td>'+ listdata[i].addtime +'</td>'
                            	         +     '<td>'+ listdata[i].username +'</td>'
                            	         +     '<td>'+ listdata[i].firstpayment +'</td>'
                            	         +     '<td>'+ listdata[i].finalpayment +'</td>'
	                                     +   '</tr>';
                            }
                            // 为了测试，延迟1秒加载
                            setTimeout(function(){
                            	...
                            	//每次数据加载完，必须重置
                                 ...
                                 $('.lb-list').on('click', function() {
                                	 var dispatchcode = $(this).find("input[type=hidden]").val();
                                	 
                                	 //查看工单详情
                                	 lookUserdispatch(dispatchcode);
     		                     });
	...
	function lookUserdispatch(dispatchcode) {
		
		$("#addForm").attr("action", "mobilebusiness/saveDispatchdataInit?rid="+Math.random()+"&dispatchcode="+dispatchcode);
		$("#addForm").submit();
