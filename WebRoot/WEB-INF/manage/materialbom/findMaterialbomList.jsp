...
<body>
    ...
    <div class="seh-box">
        <form action="" method="post" id="searchForm" name="searchForm">
            <table width="100%">
				<tr>
					<td align="right" width="10%">材料型号：</td>
					<td width="20%">
						 <select id="materialcode" name="materialcode" class="select" style="width:300px;"  onchange="initBom();">
							<option value=""  <c:if test="${materialbom.materialcode == ''}">selected</c:if>>请选择</option>
		    			    <option value="0066" <c:if test="${materialbom.materialcode == '0066' }">selected</c:if>>内主板(0066)</option>
			                <option value="0067" <c:if test="${materialbom.materialcode == '0067' }">selected</c:if>>备用CPU板(0067)</option>
			                <option value="0063" <c:if test="${materialbom.materialcode == '0063' }">selected</c:if>>外按键板(0063)</option>
			             </select>
					</td>
					<td align="right">
					    <!-- 
...
<script type="text/javascript">
    //查询
	function queryBom(){
		$("#searchForm").attr("action", "materialbom/findByList");
		$("#searchForm").submit();
	}	
	...
	function addBom(){
		 var materialcode = $("#materialcode").val();
	  	 if (materialcode != '') {
	  	 	var url = 'materialbom/addInit?materialcode=' + materialcode;
	  		var bommain = $('#bommain');
	...
	*/
	function updateBom(id){
		$("#searchForm").attr("action", "materialbom/updateInit?id="+id+"&pager_offset="+'${bom.pager_offset}');
		$("#searchForm").submit();
	}
    ...
    function initProductmodel(){
    	...
    	$.getJSON('product/getProductCombogridJson?rid='+Math.random(), data, function (productJson) {
		     ...
		     for (var key in productJson) {
		        options += '<option value="' + productJson[key].materialcode + '">' + productJson[key].productname + '(' + productJson[key].materialcode + ')' + '</option>';
		     }
		     $('#materialcode').children().remove();
		     $('#materialcode').append(options);
		     $('#materialcode').val('${bom.materialcode}');
		     initBom();
	   });
    }
	...
	*/
	function deleteBom(id){
		$.dialog.confirmMultiLanguage('<spring:message code="page.confirm.execution"/>?', 
			...
			function(){ 
				$("#searchForm").attr("action", "materialbom/delete?id="+id+"&pager_offset="+'${bom.pager_offset}'+"&rid="+Math.random());
				$("#searchForm").submit();
			}, function(){
						});
	 ...
	 * 加载区域列表
	 */
	function loadTopology(materialcode) {
	    var setting = {
		    view: {
		      selectedMulti: false,
		      showTitle: false
	    ...
	    var url = 'materialbom/getTreeBom';
	    
		$.getJSON(url, data, function (jsonMsg) {
			
		    $.fn.zTree.init($('#treeDemo'), setting, jsonMsg);
		    
		    initSelectedNode(materialcode);
	    });
	}
    ...
    //初始化选择节点
    function initSelectedNode(materialcode) {
	...
	function menuClick(item) {
		 var treeObj = $.fn.zTree.getZTreeObj('treeDemo'),
		  ...
		  var nodeType = node.type,
		      ...
		      url = '';
		  if (nodeType == 1) { // 网络
		      url = 'materialbom/addInit?materialcode=' + id;
		      bommain.attr('src', url);
		  } else if (nodeType == 2) { //区域
			  if (item.id == 'add') {
			      url = 'materialbom/addInit?pid=' + id;
			      bommain.attr('src', url);
			   } else if (item.id == 'delete') {
			      url = 'materialbom/deleteBatchByCode';
